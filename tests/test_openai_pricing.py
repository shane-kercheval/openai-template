"""Tests for openai_pricing.py."""
import pytest
from source.library.openai_pricing import PRICING_LOOKUP, EmbeddingModels, _encode, \
    num_tokens, cost, InstructModels


def test__cost__instruct_models() -> None:
    """Tests the cost function for the instruct models."""
    ...
    def _verify_cost(model: InstructModels) -> None:
...
def test__cost__embedding_models() -> None:
    """Tests the cost function for the embedding models."""
    test_value = "This is a string"
    ...
    def _verify_cost(model: EmbeddingModels) -> None:
        pricing = PRICING_LOOKUP[model]
        assert cost(test_value, model=model) == len(encoding) * pricing.price_per_tokens / pricing.per_x_tokens  # noqa
...
def test__cost__invalid_type() -> None:
    """Validate that the cost function raises an error when an unexpected type is passed."""
